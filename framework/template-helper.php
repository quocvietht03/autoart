...
if (! function_exists('autoart_setup')) {
	function autoart_setup()
	{
		/* Load textdomain */
		...
		register_nav_menus(array(
			'primary_menu'   => esc_html__('Primary Menu', 'autoart'),
		...
		add_theme_support('custom-background', apply_filters('autoart_custom_background_args', array(
			'default-color' => 'f5f5f5',
		)));

		/* Add support for featured content. */
		add_theme_support('featured-content', array(
			'featured_content_filter' => 'autoart_get_featured_posts',
			'max_posts' => 6,
		));
...
if (! function_exists('autoart_wp_title')) {
	function autoart_wp_title($title, $sep)
	{
		...
		if ($paged >= 2 || $page >= 2) {
			$title = sprintf(esc_html__('Page %s', 'autoart'), max($paged, $page)) . " $sep $title";
		}
		return $title;
	}
	add_filter('wp_title', 'autoart_wp_title', 10, 2);
}

/* Logo */
if (!function_exists('autoart_logo')) {
	function autoart_logo($url = '', $height = 30)
	{
		if (!$url) {
			$url = get_template_directory_uri() . '/assets/images/logo.png';
		}
		echo '<a href="' . home_url('/') . '"><img class="logo" style="height: ' . esc_attr($height) . 'px; width: auto;" src="' . esc_url($url) . '" alt="' . esc_attr__('Logo', 'autoart') . '"/></a>';
	}
}

/* Nav Menu */
if (!function_exists('autoart_nav_menu')) {
	function autoart_nav_menu($menu_slug = '', $theme_location = '', $container_class = '')
	{
...
/* Page title */
if (!function_exists('autoart_page_title')) {
	function autoart_page_title()
	{
		ob_start();
		if (is_front_page()) {
			esc_html_e('Home', 'autoart');
		} elseif (is_home()) {
			esc_html_e('Blog', 'autoart');
		} elseif (is_search()) {
			esc_html_e('Search', 'autoart');
		} elseif (is_404()) {
			esc_html_e('404', 'autoart');
		} elseif (is_page()) {
			echo get_the_title();
		} elseif (is_archive()) {
			if (is_category()) {
				...
				esc_html_e('Our Dealers', 'autoart');
				...
				printf(__('Author: %s', 'autoart'), '<span class="vcard">' . get_the_author() . '</span>');
			} elseif (is_day()) {
				printf(__('Day: %s', 'autoart'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			} elseif (is_month()) {
				printf(__('Month: %s', 'autoart'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			} elseif (is_year()) {
				printf(__('Year: %s', 'autoart'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			} elseif (is_tax('post_format', 'post-format-aside')) {
				esc_html_e('Aside', 'autoart');
			} elseif (is_tax('post_format', 'post-format-gallery')) {
				esc_html_e('Gallery', 'autoart');
			} elseif (is_tax('post_format', 'post-format-image')) {
				esc_html_e('Image', 'autoart');
			} elseif (is_tax('post_format', 'post-format-video')) {
				esc_html_e('Video', 'autoart');
			} elseif (is_tax('post_format', 'post-format-quote')) {
				esc_html_e('Quote', 'autoart');
			} elseif (is_tax('post_format', 'post-format-link')) {
				esc_html_e('Link', 'autoart');
			} elseif (is_tax('post_format', 'post-format-status')) {
				esc_html_e('Status', 'autoart');
			} elseif (is_tax('post_format', 'post-format-audio')) {
				esc_html_e('Audio', 'autoart');
			} elseif (is_tax('post_format', 'post-format-chat')) {
				esc_html_e('Chat', 'autoart');
			} else {
				esc_html_e('Archive', 'autoart');
			}
		} else {
			echo get_the_title();
		}
...
/* Page breadcrumb */
if (!function_exists('autoart_page_breadcrumb')) {
	function autoart_page_breadcrumb($home_text = 'Home', $delimiter = '-')
	{
		global $post;

		if (is_front_page()) {
			echo '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Front Page', 'autoart');
		} elseif (is_home()) {
			echo  '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Blog', 'autoart');
		} else {
		...
		if (is_category()) {
			...
			echo '<span class="current">' . esc_html__('Search results for: ', 'autoart') . get_search_query() . '</span>';
		} elseif (is_day()) {
			...
			echo '<span class="current">' . esc_html__('Articles posted by ', 'autoart') . $userdata->display_name . '</span>';
		} elseif (is_404()) {
			echo '<span class="current">' . esc_html__('Error 404', 'autoart') . '</span>';
		}

		if (get_query_var('paged')) {
			if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) echo ' (';
			echo ' <span class="bt-deli">' . $delimiter . '</span> ' . esc_html__('Page', 'autoart') . ' ' . get_query_var('paged');
			if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) echo ')';
		}
	}
}
...
if (! function_exists('autoart_post_nav')) {
	function autoart_post_nav()
	{
...
?>
		<nav class="bt-post-nav clearfix">
			<?php
			previous_post_link('<div class="bt-post-nav--item bt-prev"><span>' . esc_html__('Previous Post', 'autoart') . '</span><h3>%link</h3></div>');
			next_post_link('<div class="bt-post-nav--item bt-next"><span>' . esc_html__('Next Post', 'autoart') . '</span><h3>%link</h3></div>');
			?>
		</nav>
	<?php
	}
...
if (! function_exists('autoart_paginate_links')) {
	function autoart_paginate_links($wp_query)
	{
...
if (! function_exists('autoart_paging_nav')) {
	function autoart_paging_nav()
	{
